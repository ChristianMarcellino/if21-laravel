                  ...
                  <form action="{{ route('mahasiswa.update', $mahasiswa->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                      <div class="mb-3">
                        ...
                        <input id='nama' type="text" class="form-control" name="nama" value="{{ old('nama') ? old('nama') : $mahasiswa->nama}}">
                        @error('nama')
                      ...
                      <div class="mb-3">
                        ...
                        <br>
                        <input class="form-check-input" value="L" id="L" type="radio" name="jk" {{ old('jk') == 'L' ? 'checked' : ($mahasiswa->jk == 'L' ? 'checked' : null) }}>
                        <label for="jk" class="form-check-label">Laki-Laki</label>
                        <input class="form-check-input" type="radio" id="P" value="P" name="jk" {{ old('jk') == 'P' ? 'checked' : ($mahasiswa->jk == 'P' ? 'checked' : null) }}>
                      ...
                      <div class="mb-3">
                        ...
                        @foreach ($prodi as $item)
                          <option value="{{ $item -> id }} {{ old('$item->nama') ? 'selected' : null}}">{{ $item->nama }}</option>
                        @endforeach
                      </select>
                      </div>
                    </div>
